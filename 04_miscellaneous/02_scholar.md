...
by respectively calling `{% raw %}{% bibliography --file books %}{% endraw %}` and
`{% raw %}{% bibliography --file papers %}{% endraw %}`. For example, you could have a file `references.md`
...
By default, the template is `{% raw %}{{reference}}{% endraw %}` – this renders only the
...
`{% raw %}{% cite ruby microscope %}{% endraw %}` would produce a cite tag like:
...
`...as Matz explains {% raw %}{% cite ruby -A -l 42 %}{% endraw %}` would produce something
...
`-l` or `--locator` option. For example, `{% raw %}{% cite ruby -l 23-5 %}{% endraw %}` would
produce a citation like `(Matsumoto, 2008, pp. 23-5)`.

When quoting multiple items (see above) you can add multiple locators after
the list of ids. For example, `{% raw %}{% cite ruby microscope -l 2 -l 24 & 32 %}{% endraw %}`.
...
using `{% raw %}{% reference ruby %}{% endraw %}` anywhere in your page, it will print
...
When you have multiple `{% raw %}{% bibliography %}{% endraw %}` sections in one file,
